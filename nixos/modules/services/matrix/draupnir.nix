...
in
{
  options.services.draupnir = {
    ...
    accessTokenFile = mkOption {
      ...
      default = null;
      description = lib.mdDoc ''
        File containing the matrix access token for the `draupnir` user.
      '';
    };
      ...
      type = types.submodule {
        options = {
          ...
          passwordFile = mkOption {
            ...
            default = null;
            description = lib.mdDoc ''
              File containing the matrix password for the `draupnir` user.
            '';
          };
  ...
  config = mkIf config.services.draupnir.enable {
    ...
    systemd.services.draupnir = {
      ...
      wants = [ "network-online.target" ] ++ optionals (cfg.pantalaimon.enable) [ "pantalaimon-draupnir.service" ];
      after = [ "network-online.target" "matrix-synapse.service" ] ++ optionals (cfg.pantalaimon.enable) [ "pantalaimon-draupnir.service" ];
      wantedBy = [ "multi-user.target" ];

      serviceConfig = {
        ExecStart = ''${pkgs.draupnir}/bin/draupnir'';
